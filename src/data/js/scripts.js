...
const traeHost = () => {
  if (window.location.hostname == "localhost") {
    return "192.168.20.160"; // TODO: Modo Desarrollo IPv4 del Equipo en su red
  } else {
    return window.location.hostname; // TODO: window.location.hostname; Modo producción ip real del Equipo
  }
};

const wsURL = `ws://${traeHost()}/ws`;
let reConnect = false;
let tt = 0;
let ws = "";
...
const initWS = () => {
  ...
  ws.onmessage = (m) => {
    ...
    if (json.type == "info") {
      mensaje("top-end", json.icon, json.msg, 2000);
    }
    if (json.type == "update") {
      document.getElementById("fw_Signal").style.width = json.msg + "%";
    ...
    if (json.type == "rs232") {
      ...
      // Agregar timestamp y nueva línea
      const now = new Date().toLocaleTimeString();
      logElement.textContent += `[${now}] ${json.msg}\n`;
    ...
    if (pathname == "/") {
      if (json.type == "data") {
        ...
        if (json.mqtt_status == "CONECTADO") {
          // estado
          document.getElementById("mqtt_on").innerHTML = "Online";
          // clase progress
          document
            .getElementById(`progress_mqtt_on`)
            .classList.remove("bg-danger");
          document
            .getElementById(`progress_mqtt_on`)
            .classList.add("bg-success");
          // clase span
          document.getElementById(`mqtt_on`).classList.remove("text-danger");
          document.getElementById(`mqtt_on`).classList.add("text-success");
          // span
          document
            .getElementById(`mqtt_status`)
            .classList.remove("badge-danger");
          document.getElementById(`mqtt_status`).classList.add("badge-success");
        } else {
          // estado
          document.getElementById("mqtt_on").innerHTML = "Offline";
          // clase progress
          document
            .getElementById(`progress_mqtt_on`)
          ...
          // clase span
          document.getElementById(`mqtt_on`).classList.remove("text-success");
          document.getElementById(`mqtt_on`).classList.add("text-danger");
          // span
          document
            .getElementById(`mqtt_status`)
            .classList.remove("badge-success");
          document.getElementById(`mqtt_status`).classList.add("badge-danger");
        }

        if (json.wifi_status == "CONECTADO") {
          document.getElementById("wifi_on").innerHTML = "Online";
          document
            .getElementById(`progress_wifi_on`)
            .classList.remove("bg-danger");
          document
            .getElementById(`progress_wifi_on`)
            .classList.add("bg-success");
          document.getElementById(`wifi_on`).classList.remove("text-danger");
          document.getElementById(`wifi_on`).classList.add("text-success");
          document
            .getElementById(`wifi_status`)
            .classList.remove("badge-danger");
          document.getElementById(`wifi_status`).classList.add("badge-success");
        } else {
          document.getElementById("wifi_on").innerHTML = "Offline";
          document
            .getElementById(`progress_wifi_on`)
            .classList.remove("bg-success");
          document
            .getElementById(`progress_wifi_on`)
            .classList.add("bg-danger");
          document.getElementById(`wifi_on`).classList.remove("text-success");
          document.getElementById(`wifi_on`).classList.add("text-danger");
          document
            .getElementById(`wifi_status`)
            .classList.remove("badge-success");
          document.getElementById(`wifi_status`).classList.add("badge-danger");
        }
        ...
        document.getElementById("wifi_percent").innerHTML =
          json.wifi_percent + " %"; // % de la señal WIFI

        // progressbar
        document.getElementById("wifi_Signal").style.width =
          json.wifi_percent + "%";
        document.getElementById("wifi_label").innerHTML =
          json.wifi_percent + "%";
        document.getElementById("spiffs_Signal").style.width =
          json.spiffs_used + "%";
        document.getElementById("spiffs_label").innerHTML =
          json.spiffs_used + "%";
        document.getElementById("spiffs_info").innerHTML =
          json.spiffs_used + "%";
        document.getElementById("ram_Signal").style.width =
          json.ram_available + "%";
        document.getElementById("ram_label").innerHTML =
          json.ram_available + "%";
...
const expresiones = {
  ...
  ServerPort:
    /^(6553[0-5]|655[0-2][0-9]|65[0-4][0-9]{2}|6[0-4][0-9]{3}|[0-5]?([0-9]){0,3}[0-9])$/, // Puertos desde 0 a 65535.
  ...
  HexCommand: /^([0-9A-Fa-f]{2})(\s[0-9A-Fa-f]{2})*$/,
};
...
function validarFormulario(e) {
  ...
  switch (e.target.name) {
    case "mqtt_id":
      validarCampo(
        expresiones[e.target.dataset.expresion],
        e.target,
        e.target.name,
      );
      break;
    ...
    case "id":
      ...
      break;
    case "commandInput":
      const valido = expresiones[e.target.dataset.expresion].test(
        e.target.value,
      );
      validarCampo(
        expresiones[e.target.dataset.expresion],
      ...
      if (document.getElementById("page").innerHTML == "Consola RS232") {
        const checksumLabel = document.getElementById("form_commandChecksum");
        if (valido) {
          const cs = calcularChecksumXOR(e.target.value);
          checksumLabel.textContent = cs
            ? `Checksum XOR: ${cs}`
            : "Formato inválido";
        } else {
          checksumLabel.textContent = "Formato inválido";
        }
      }
      break;
  }
...
const validarCampo = (expresion, input, campo) => {
  ...
  if (expresion.test(input.value) && input.value != "") {
    document
      .querySelector(`#form_${campo}`)
      .classList.remove("formulario_input-error-activo");
    document
      .querySelector(`#form_${campo}`)
      .classList.add("formulario_input-error");
    campos[campo] = true;
  } else {
    document
      .querySelector(`#form_${campo}`)
      .classList.remove("formulario_input-error");
    document
      .querySelector(`#form_${campo}`)
...
const validarPassword = () => {
  ...
  if (inputNewPassword.value == inputNewPasswordConfirm.value) {
    document
      .querySelector(`#form_c_new_www_password`)
      .classList.add("formulario_input-error");
    document
      .querySelector(`#form_c_new_www_password`)
      .classList.remove("formulario_input-error-activo");
...
if (
  ...
  document.addEventListener("DOMContentLoaded", function (event) {
    document
      .getElementById("form")
      .addEventListener("submit", manejadorValidacion);
  });
}
...
function manejadorValidacion(e) {
  ...
  if (page == "Configuración del Broker MQTT") {
    ...
    if (
      campos.wifi_ssid &&
      campos.wifi_passw &&
      campos.wifi_ip_static &&
      campos.wifi_subnet &&
      campos.wifi_gateway &&
      campos.wifi_primaryDNS &&
      campos.wifi_secondaryDNS &&
      campos.ap_nameap &&
      campos.ap_passwordap &&
      campos.ap_canalap &&
      campos.ap_connetap
    ) {
      document
    ...
    if (
      campos.www_username &&
      campos.www_password &&
      campos.new_www_username &&
      campos.new_www_password
    ) {
      document
        .getElementById("formulario_mensaje")
        .classList.remove("formulario_mensaje-activo");
  ...
  } else if (page == "Consola RS232") {
    if (campos.commandInput) {
      document
        .getElementById("formulario_mensaje")
        .classList.remove("formulario_mensaje-activo");
      SweetAlert("¿Enviar comando?", page, "question", this);
    } else {
      document
        .getElementById("formulario_mensaje")
        .classList.add("formulario_mensaje-activo");
      mensaje(
        "top-end",
        "error",
        "Formato inválido. Usa pares hex separados por espacio (ej: 00 AB FF)",
        3000,
      );
    }
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      if (data.code == 1) {
        ...
        for (const { n, ssid, rssi, secure, bssid, channel } of data.data) {
          if (parseInt(rssi) >= -67) {
            mitabla.innerHTML +=
              `  <tr class="bg-blue text-light">
                                                        ...
                                                        <td><button class="btn btn-outline-success" onclick='selectWiFi("` +
              ssid +
              `");'><span class="text-light"><i class="fa fa-magnet"></i> Seleccionar</span></button></td>
                                                    </tr>
                                                `;
...
const uploadSettings = () => {
  ...
  if (file != undefined) {
    if (
      file["type"] != "text/json" &&
      file["name"] != "settingwifi.json" &&
      file["name"] != "settingmqtt.json" &&
      file["name"] != "settingrelays.json" &&
      file["name"] != "settingadmin.json"
    ) {
      mensaje(
        "top-end",
        "warning",
        `"Solo se permiten los archivos de configuración de la lista"`,
        5000,
      );
      return;
    }
    ...
    mensaje(
      "top-end",
      "error",
      `"Tienes que seleccionar antes un archivo"`,
      5000,
    );
    return;
  }
  ...
  fetch(url, requestOptions)
    ...
    .then((datos) => {
      if (datos.save) {
        ...
        mensaje(
          "top-end",
          "success",
          `"Archivo ${file["name"]} almacenado correctamente"`,
          5000,
        );
      } else {
    ...
    .catch((error) => {
      mensaje(
        "top-end",
        "error",
        `"Error al guardar el archivo (${file["name"]}): ${error}"`,
        5000,
      );
    });
};
  ...
  if (file != undefined) {
    if (file["type"] != "application/octet-stream") {
      mensaje(
        "top-end",
        "warning",
        `"Solo se permiten archivos binarios ( *.bin ) "`,
        5000,
      );
      return;
      ...
      mensaje(
        "top-end",
        "warning",
        `"El tamaño máximo del archivo tiene que ser de ( 1.5 MB )"`,
        5000,
      );
      return;
    }
    ...
    .then((datos) => {
      if (datos.save) {
        ...
        mensaje(
          "top-end",
          "success",
          `"${datos.type} actualizado correctamente"`,
          5000,
        );
...
function calcularChecksumXOR(hexString) {
  // Divide la cadena en pares hex separados por espacio
  const bytes = hexString.trim().split(/\s+/);
  let checksum = 0;

  for (let b of bytes) {
    if (/^[0-9A-Fa-f]{2}$/.test(b)) {
      checksum ^= parseInt(b, 16); // XOR acumulado
    } else {
