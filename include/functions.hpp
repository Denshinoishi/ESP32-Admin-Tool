...
// Envió de Comandos por RS232 usando driver UART
void sendRS232Command(String payload) {
    // separar por espacios
    int start = 0;
    while (start < payload.length()) {
        int end = payload.indexOf(' ', start);
        if (end == -1) end = payload.length();
        String byteStr = payload.substring(start, end);
        int value = strtol(byteStr.c_str(), NULL, 16);

        // enviar cada byte por UART driver
        uint8_t b = (uint8_t)value;
        uart_write_bytes(UART_NUM_2, (const char*)&b, 1);
